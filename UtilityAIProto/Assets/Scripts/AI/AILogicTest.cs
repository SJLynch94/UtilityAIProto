...
public class AILogicTest : MonoBehaviour
{
                ...
                for (var i = 0; i < mEnemies.Count; ++i)
                {
                    if (mEnemies.Values[i].gameObject != null)
                    {
                        if (mEnemies.Values[i].GetComponent<AILogic>())
                        {
                            ...
                            {
                                mDistance.Value += 100.0f * UtilityAIProto.UAI_Time.MyTime;
                                ...
                                {
                                    mAttackDistance.Value += 150.0f * UtilityAIProto.UAI_Time.MyTime;
    ...
    void Fire()
    {
        if (mCurrentTarget)
        {
            if (mCurrentTarget.GetComponent<AILogic>())
            {
                ...
                {
                    mAttackDistance.Value -= 100.0f * UtilityAIProto.UAI_Time.MyTime;
                    return;
                }
            }
        }
